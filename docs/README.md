...
`edu_crawler` 是一支單機、單 process 的非同步網頁爬蟲，本目錄收錄它的系統
設計與一次 48 小時無人值守運行的實測結果。文件分為兩份編號正文與一份同內容
的簡報形式，編號對應檔名前綴，章節編號（如 §1.4、§2.3.1）在三份文件之間可
互相引用。
...
python scripts/generate_seeds.py      # 產生 seeds.txt（1,000 個網址）
...
`results/endstate-60h.json` 為 60.2 小時 process 結束時的佇列與網域狀態。
