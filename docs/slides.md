...
| **1.90%** | 兩者之比；98.07% 的發現在試圖進入佇列時就因佇列已滿被丟棄 |
...
## 附錄：去重發生在入列時，所以 66,643,054 是發現次數而不是網址數

| 呼叫者 | 方法 | 佇列滿時 |
|---|---|---|
| 種子清單載入（`add_many()`） | `add()` | 等待佇列空出位置 |
| 連結追蹤（`_follow_links()`） | `try_add()` | 立刻放棄該連結並計數 |

- 入列前先正規化網址再查 `_seen` 去重。但**被放棄的連結不會寫入 `_seen`**，所以同一個網址下次再被發現時會再被計一次——這就是那 6,664 萬是「發現次數」的原因
- 不重複的已發現網址數落在 **1,287,294 與 66,643,054 之間**，無法由本次輸出重建：被丟掉的網址沒有留下任何紀錄

**連結追蹤的呼叫端本身就是那個數量有限的 worker pool 裡的一員，而佇列唯一的排空途徑是 worker 取出網址；在已滿的有界佇列上做阻塞式入列，等於生產者在等一個就是自己的消費者。**

<!--
這一頁是「discovered 有沒有去重」的正式答案，被問到就翻到這裡。

去重發生在入列時而非抓取時，因此「在 _seen 裡」等價於「至多被入列過一次」。
但 try_add() 在佇列滿時於 frontier.py:118-120 直接返回 False，不寫 _seen，
放棄的語意是「這次不排程」而不是「永久排除」——代價就是同一個網址可以被
重複計入 crawler_links_dropped_queue_full_total。

還有一個反方向的偏差要一起講：連結的網域若已達 --max-pages-per-domain 20，
在 worker.py:99 就被跳過，根本沒進到計數器。所以真正抽出來的 <a href> 比
6,664 萬更多。這個數字同時被高估（重複計數）和低估（上限過濾），兩邊都要認。

要把不重複發現數量出來只要一行：丟棄時除了計數，把網址也記進一個 set 或寫進檔案。
這和主線最後一頁的 except 與存活 worker gauge 是同一類問題——這支爬蟲看不見自己。
